...
namespace SignalRBorsa.Controllers
{
    public class HomeController : Controller
    {
        //
        // GET: /Home/
        public async void GetCanliBorsa()
        {
    ...
    //        var response = await client.GetAsync("https://www.sabah.com.tr/finans/pages/canliborsa_data?yil=2023&ay=2&gun=3&"+saat+"&"+dakika+"&"+saniye+"&"+"rnd=9887415821");
    //        var apiData = await response.Content.ReadAsStringAsync();
    //        apiData = apiData.Replace("callbackRT(", "").Replace("\"});","\"}");

    //        var arr = apiData.Split('~');
    //        List<Model> asd = new List<Model>();
    ...
    //            Model de = new Model();
    //            de.Name = arr2[0];
    //            de.Son = double.Parse(arr2[1]);
    //            de.Ort = double.Parse(arr2[2]);
    //            //de.HacimTl = double.Parse(arr2[3]);
    //            de.Yuzde = double.Parse(arr2[4]);
    //            asd.Add(de);
    //        }
            

    //        var model = JsonConvert.DeserializeObject<Model>(apiData);
